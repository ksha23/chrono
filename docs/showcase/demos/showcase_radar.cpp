...
int main(int argc, char** argv) {
    ...
    auto addBox = [&](ChVector3d sz, ChVector3d pos) {
        ...
        b->SetFixed(true); b->SetPos(pos); sys.Add(b);
    };
    ...
    auto radar = chrono_types::make_shared<ChRadarSensor>(
        audi.GetChassisBody(), 10.0f, radar_off, RW, RH, HFOV, VFOV, MAXD, 0.6f /*clip_near*/);
    ...
    while (time < 6.0) {
        ...
        if (buf && buf->Buffer && buf->TimeStamp > last_ts + 1e-6) {
            ...
            os.write((char*)&w, 4); os.write((char*)&h, 4); os.write((char*)hdr, sizeof(hdr));
            ...
            float Rf[9];
            for (int rr = 0; rr < 3; ++rr) for (int cc = 0; cc < 3; ++cc) Rf[rr*3+cc] = (float)R(rr, cc);
            os.write((char*)Rf, sizeof(Rf));
            size_t n = (size_t)w * h;
            for (size_t i = 0; i < n; ++i) {
                const RadarReturn& r = buf->Buffer[i];
                float rec[7] = {r.range, r.azimuth, r.elevation,
                                r.doppler_velocity[0], r.doppler_velocity[1], r.doppler_velocity[2], r.amplitude};
