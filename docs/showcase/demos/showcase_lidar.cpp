...
int main(int argc, char** argv) {
    ...
    auto lidar = chrono_types::make_shared<ChLidarSensor>(
        audi.GetChassisBody(), 10.0f, ChFrame<double>(ChVector3d(0, 0, 1.9), QUNIT),
        LW, LH, HFOV, VMAX, VMIN, MAXD);
    ...
    while (time < 6.0) {
        ...
        if (buf && buf->Buffer && buf->TimeStamp > last_ts + 1e-6) {
            ...
            size_t n = (size_t)w * h;
            for (size_t i = 0; i < n; ++i) { float r = buf->Buffer[i].range; os.write((char*)&r, 4); }
            for (size_t i = 0; i < n; ++i) { float in_ = buf->Buffer[i].intensity; os.write((char*)&in_, 4); }
        }
        time += step;
    }
