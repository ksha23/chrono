...
int main(int argc, char** argv) {
    ...
    auto cam = chrono_types::make_shared<ChCameraSensor>(
        body, 10.0f,
        ChFrame<double>(cam_off, QuatFromAngleZ(std::atan2(cd.y(), cd.x())) * QuatFromAngleY(-std::asin(cd.z()))),
    ...
    auto tach = chrono_types::make_shared<ChTachometerSensor>(spindle, 20.0f, ChFrame<double>(),
                                                              ChTachometerSensor::Axis::Y);
    ...
    printf("%6s %28s %28s %30s %26s %8s\n", "t[s]", "accel [m/s^2]", "gyro [rad/s]", "magnet [T]",
           "GPS lat/lon/alt", "rpm");
    ...
    while (time < 4.0) {
        ...
        if (a && a->Buffer) got_acc++;
        if (g && g->Buffer) got_gyro++;
        if (m && m->Buffer) got_mag++;
        if (p && p->Buffer) got_gps++;
        if (t && t->Buffer) got_tach++;
        ...
        if (time >= next_print && a && a->Buffer && g && g->Buffer && m && m->Buffer && p && p->Buffer && t && t->Buffer) {
            printf("%6.2f  %8.3f %8.3f %8.3f  %8.4f %8.4f %8.4f  %9.2e %9.2e %9.2e  %10.6f %11.6f %7.1f %8.1f\n",
                   ...
                   g->Buffer[0].Roll, g->Buffer[0].Pitch, g->Buffer[0].Yaw,
                   m->Buffer[0].X, m->Buffer[0].Y, m->Buffer[0].Z,
                   p->Buffer[0].Latitude, p->Buffer[0].Longitude, p->Buffer[0].Altitude,
                   t->Buffer[0].rpm);
            next_print += 0.5;
        }
        time += step;
