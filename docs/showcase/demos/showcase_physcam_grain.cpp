// SHOWCASE (METAL backend): gritty sensor look -- VIGNETTE + GAUSSIAN NOISE + low EXPOSURE.
// A stationary Audi on flat terrain under the shipped sky_2_4k HDR, camera orbiting. The frame is darkened
// (low exposure), corners fall off (vignette), and gaussian sensor noise is added for a raw, low-light
// sensor feel. HEADLESS: writes 150 PNGs to demos_live/showcase_out/physcam_grain/ then returns.
#include <cmath>
#include <cstdio>
#include <cstdlib>
#include <string>
...
int main(int argc, char** argv) {
    ...
    // Gritty low-light sensor look: darker exposure, strong corner vignette, gaussian sensor noise.
    manager->scene->SetExposure(0.6f);        // slightly under-exposed
    manager->scene->SetVignette(0.6f);        // pronounced corner falloff
    manager->scene->SetSensorNoise(0.03f);    // visible gaussian grain
    ...
    auto cam = chrono_types::make_shared<ChCameraSensor>(
        ...
        (float)(CH_PI / 3), 1 /*ss*/, CameraLensModelType::PINHOLE, false /*GI*/, false /*denoiser*/);
    cam->SetName("physcam_grain");
    cam->PushFilter(chrono_types::make_shared<ChFilterSave>("demos_live/showcase_out/physcam_grain/"));
    manager->AddSensor(cam);

    printf("Gritty sensor (vignette/noise/low-exposure) showcase (Metal). PNGs -> demos_live/showcase_out/physcam_grain/\n");
