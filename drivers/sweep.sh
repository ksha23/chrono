...
OUT=$W/logs/sweep_$(date +%Y%m%d_%H%M%S).txt
cd $W/run
...
for rep in $(seq 1 $REPS); do
  scen smc10k mcore_granular PROF_METHOD=SMC PROF_N=10000 PROF_WARM=60 PROF_STEPS=40 PROF_PROBE_RC=50
  scen smc40k mcore_granular PROF_METHOD=SMC PROF_N=40000 PROF_WARM=60 PROF_STEPS=40 PROF_PROBE_RC=50
  scen nsc5k mcore_granular PROF_METHOD=NSC PROF_N=5000 PROF_WARM=20 PROF_STEPS=10 PROF_PROBE_RC=10
  scen cratering demo_MCORE_cratering PROF_WARM=4000 PROF_STEPS=300
  ...
  scen callbackSMC demo_MCORE_callbackSMC PROF_WARM=100 PROF_STEPS=2000
  scen callbackNSC demo_MCORE_callbackNSC PROF_WARM=100 PROF_STEPS=2000
done
echo "wrote $OUT"; touch $W/logs/sweep_done
