...
double sizeX = 1e-2 * prof::env_int("PROF_SIZE_CM", 4);      // length in x direction
double sizeY = 1e-2 * prof::env_int("PROF_SIZE_CM", 4);      // depth in y direction
...
int numLayers = prof::env_int("PROF_LAYERS", 5);
...
int main(int argc, char* argv[]) {
    ...
    threads = prof::threads(std::min(threads, ChOMP::GetNumProcs()));
    sys->SetNumThreads(prof::threads(threads));
