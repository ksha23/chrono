...
void Render(const BuildFn& build,
            const ChFrame<double>& cam_pose,
            unsigned W,
            unsigned H,
            float hfov,
            const RenderOpts& opt,
            Frame* out_last,
            Frame* out_first = nullptr) {
    ChSystemNSC sys;
    ...
    auto cam = chrono_types::make_shared<ChCameraSensor>(mount, 100.f, cam_pose, W, H, hfov, opt.supersample,
                                                         CameraLensModelType::PINHOLE, opt.use_gi, false,
                                                         Integrator::LEGACY, opt.gamma, false);
    cam->SetLag(0);
...
std::shared_ptr<ChBody> BuildShadowScene(ChSystemNSC& sys,
    ...
    if (area_extent <= 0.0)
        ...
        manager->scene->AddRectangleLight({2.f, 3.f, 0.f}, ChColor(14.f, 14.f, 14.f), 80.f,
                                          {0.f, (float)area_extent, 0.f}, {0.f, 0.f, (float)area_extent});
...
TEST(MetalStochastic, gi_supersampling_reduces_noise) {
    ...
    EXPECT_LT(ratio, 0.75) << "supersampling did not reduce Monte Carlo noise (ratio " << ratio
                           << "); the per-sample RNG state is most likely not advancing between samples";
...
TEST(MetalStochastic, area_light_penumbra_is_wider_than_point_light) {
    ...
    Frame hard, soft;
    ASSERT_NO_FATAL_FAILURE(Render([](ChSystemNSC& s,
                                      std::shared_ptr<ChSensorManager> m) { return BuildShadowScene(s, m, 0.0); },
                                   ChFrame<double>(), kW, kH, kHFOV, opt, &hard));
    ASSERT_NO_FATAL_FAILURE(Render([](ChSystemNSC& s,
                                      std::shared_ptr<ChSensorManager> m) { return BuildShadowScene(s, m, 2.0); },
                                   ChFrame<double>(), kW, kH, kHFOV, opt, &soft));
    ...
    EXPECT_GT(ws, 3.0 * std::max(wh, 1.0))
        << "the area light's penumbra (" << ws << " px) is not meaningfully wider than the point light's " << wh
...
TEST(MetalStochastic, depth_of_field_blurs_out_of_focus_edges) {
    ...
    EXPECT_GT(wb, 2.5 * std::max(ws, 1.0))
        << "a 0.15 m aperture focused 37 m past the near box did not soften its silhouette ("
        << wb << " px vs " << ws << " px)";
}
...
TEST(MetalStochastic, sensor_noise_scales_with_sigma_and_preserves_the_mean) {
    ...
    for (int i = 1; i < 3; ++i) {
        ...
        std::cout << "  [noise] sigma " << sigmas[i] << " mean shift " << std::fabs(mean[i] - mean[0]) << ", allowed "
                  << tol << "\n";
