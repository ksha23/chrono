...
//       az = -hfov/2 + (i+0.5)/W * hfov              (positive az swings toward +Y/left)
...
static HitRec analytic_trace(const double o[3],
                             const double d[3],
...
static Frame frame_of(const ChFrame<double>& cf) {
    Frame f;
    const ChVector3d v[5] = {cf.GetPos(),
                             cf.TransformDirectionLocalToParent(ChVector3d(1, 0, 0)),
                             cf.TransformDirectionLocalToParent(ChVector3d(0, -1, 0)),  // screen right = -Y
                             cf.TransformDirectionLocalToParent(ChVector3d(0, 0, 1)),
                             cf.TransformDirectionLocalToParent(ChVector3d(0, 1, 0))};
...
static double beam_az(unsigned i, unsigned W, double hfov) {
    return -hfov * 0.5 + ((double)i + 0.5) / (double)W * hfov;
}
static double beam_el(unsigned j, unsigned H, double elmin, double elmax) {
    const double t = (H > 1) ? (double)j / (double)(H - 1) : 0.5;
    return elmin + t * (elmax - elmin);
}
...
static bool ambiguous_pixel(const Frame& f,
                            unsigned W,
                            unsigned H,
                            double hfov,
                            unsigned x,
...
int main(int argc, char** argv) {
    ...
    const ChFrame<double> pose_odd(ChVector3d(0.0, 0.4, 1.25),
                                   QuatFromAngleZ(75.0 * CH_DEG_TO_RAD) * QuatFromAngleY(12.0 * CH_DEG_TO_RAD));
    ...
    auto lidar = chrono_types::make_shared<ChLidarSensor>(mount, RATE, pose_fwd, LW, LH, (float)LHFOV, (float)LELMAX,
                                                          (float)LELMIN, 100.f);
    lidar->SetName("lidar_fwd");
    lidar->PushFilter(chrono_types::make_shared<ChFilterDIAccess>());
    manager->AddSensor(lidar);

    auto lidar2 = chrono_types::make_shared<ChLidarSensor>(mount, RATE, pose_odd, LW, LH, (float)LHFOV, (float)LELMAX,
                                                           (float)LELMIN, 100.f);
    ...
    auto radar =
        chrono_types::make_shared<ChRadarSensor>(mount, RATE, pose_fwd, RW, RH, (float)RHFOV, (float)RVFOV, 100.f);
    ...
    auto have = [&]() {
        return d && d->Buffer && d2 && d2->Buffer && dclamp && dclamp->Buffer && n && n->Buffer && s && s->Buffer &&
               l && l->Buffer && l2 && l2->Buffer && lshort && lshort->Buffer && rb && rb->Buffer;
    };
    ...
    printf("================================================================\n");
    if (!have()) {
        printf("  [FAIL] sensors delivered no data -- cannot run assertions\n");
        return 1;
    }
    ...
    double EFF_HFOV = HFOV;
    {
        ...
        if (std::fabs(EFF_HFOV - HFOV) >= 1e-3) {
            printf("\n   ******************************* BACKEND BUG *******************************\n");
            printf("   The renderer ignored ChDepthCamera/ChNormalCamera/ChSegmentationCamera::GetHFOV()\n");
            printf("   and fell back to its hard-coded default. metal/ChFilterMetalRTRender.mm reads the\n");
            printf("   horizontal FOV with\n");
            printf("       float hfov = 1.408f;\n");
            printf("       if (auto c = std::dynamic_pointer_cast<ChCameraSensor>(sensor)) hfov = c->GetHFOV();\n");
            printf("   but ChDepthCamera, ChNormalCamera and ChSegmentationCamera do NOT derive from\n");
            printf("   ChCameraSensor (they derive from ChMetalSensor directly), so the cast fails and every\n");
            printf("   one of them renders at 1.408 rad = %.2f deg.  Measured here: %.2f deg.\n",
                   1.408 * CH_RAD_TO_DEG, EFF_HFOV * CH_RAD_TO_DEG);
            printf("   Fix: query each concrete type, e.g.\n");
            printf("       else if (auto dc = std::dynamic_pointer_cast<ChDepthCamera>(sensor))        hfov = "
                   "dc->GetHFOV();\n");
            printf("       else if (auto nc = std::dynamic_pointer_cast<ChNormalCamera>(sensor))       hfov = "
                   "nc->GetHFOV();\n");
            printf("       else if (auto sc = std::dynamic_pointer_cast<ChSegmentationCamera>(sensor)) hfov = "
                   "sc->GetHFOV();\n");
            printf("   (the same cast chain also gates cam.lensModel / distortion, so those three cameras are\n");
            printf("    additionally stuck on PINHOLE regardless of the lens model requested).\n");
    ...
    // closed-form spot checks, so a reader can verify the numbers by hand
    {
        ...
        check("depth: centre pixel = wall face / cos(theta)", std::fabs(got - expect) < TOL_RANGE,
    ...
    }
    {
        ...
        const double got = d->Buffer[(size_t)py * W + px].depth;
        check("depth: bottom-row ray hits the floor plane", std::fabs(got - expect) < TOL_RANGE,
              fmt("%.6f m, expected %.6f (floor top z=%.2f)", got, expect, FLOOR_TOP));
    }
    ...
    printf("\n-- depth sentinel / maxDepth --\n");
    {
        ...
        check("depth: sky pixels use one consistent sentinel", (zero == 0) != (other == 0),
              fmt("%ld sky px = 0.0, %ld sky px != 0.0", zero, other));
        check_gap("depth: sky sentinel == maxDepth (OptiX)", other > 0 && zero == 0,
                  fmt("%ld sky px report 0.0 instead of maxDepth", zero),
    ...
    }
    {
        ...
        check_gap("depth: values clamped to maxDepth (OptiX)", over == 0,
                  fmt("maxDepth %.1f m, largest reported %.3f m (%ld px over)", MAXD, maxv, over),
        ...
        check_gap("depth: sky == maxDepth in a clamped camera", sky_total > 0 && sky_at_max == sky_total,
                  fmt("%ld/%ld sky px equal maxDepth", sky_at_max, sky_total), "same root cause as the gap above");
    }
    ...
    printf("\n-- normal camera --\n");
    {
        ...
        for (unsigned y = 0; y < H; ++y)
            for (unsigned x = 0; x < W; ++x) {
                ...
                const double e = std::max({std::fabs(g.normal_x - c.n[0]), std::fabs(g.normal_y - c.n[1]),
                                           std::fabs(g.normal_z - c.n[2])});
                compared++;
        ...
        check("normal: matches analytic face normal", bad == 0 && compared > 1000,
              fmt("max per-component |err| %.3e, %ld px over %.0e", maxerr, bad, TOL_NORMAL));
        check("normal: sky rays report the zero normal", sky > 0 && sky_zero == sky,
              fmt("%ld/%ld sky px = (0,0,0)", sky_zero, sky));
    }
    {
        const PixelNormal& g = n->Buffer[(size_t)(H / 2) * W + (W / 2)];
        check("normal: wall face is exactly -X",
    ...
    double cx_got = 0, cy_got = 0, cx_exp = 0, cy_exp = 0;
    {
        ...
        printf("   %ld px compared, %ld ambiguous, %ld sky\n", compared, ambiguous, sky);
        check("segmentation: class id exactly matches geometry", bad_cls == 0 && compared > 1000,
              fmt("%ld/%ld mismatched", bad_cls, compared));
        check("segmentation: instance id exactly matches geometry", bad_inst == 0 && compared > 1000,
              fmt("%ld/%ld mismatched", bad_inst, compared));
        check("segmentation: sky is class 0 / instance 0", sky > 0 && sky_zero == sky,
              fmt("%ld/%ld sky px = (0,0)", sky_zero, sky));
    }
    {
        ...
        check("segmentation: class and instance are separate",
              g.class_id == BOXES[IDX_WALL].cls && g.instance_id == BOXES[IDX_WALL].inst,
              fmt("centre = (class %u, instance %u), expected (%u, %u)", (unsigned)g.class_id,
                  (unsigned)g.instance_id, (unsigned)BOXES[IDX_WALL].cls, (unsigned)BOXES[IDX_WALL].inst));
    }
    ...
    printf("\n-- projection / FOV --\n");
    {
        ...
        check("projection: marker centre projects onto the marker", inside && cls == BOXES[IDX_MARK].cls,
              fmt("cube centre (%.1f,%.1f,%.1f) -> px (%d,%d), class %u (expected %u)", mx, my, mz, ic, ir,
                  (unsigned)cls, (unsigned)BOXES[IDX_MARK].cls));
        ...
        check("projection: marker silhouette centroid",
              mark_px_expect > 100 && std::fabs(cx_got - cx_exp) < 0.5 && std::fabs(cy_got - cy_exp) < 0.5,
        ...
        check("projection: marker silhouette area", area_err < 0.02,
              fmt("%ld px vs analytic %ld px (%.2f%% off)", mark_px, mark_px_expect, 100.0 * area_err));
    }
    ...
    auto lidar_grid = [&](const Frame& fr, const UserDIBufferPtr& buf, const char* label) {
        ...
        for (unsigned j = 0; j < LH; ++j)
            for (unsigned i = 0; i < LW; ++i) {
                const double az = beam_az(i, LW, LHFOV), el = beam_el(j, LH, LELMIN, LELMAX);
                double dir[3];
        ...
        check(fmt("lidar[%s]: range matches analytic beam", label), bad == 0 && compared > 0,
              fmt("%ld beams, max |err| %.3e m, %ld over %.0e", compared, maxerr, bad, TOL_RANGE));
        if (sky > 0)
    ...
    lidar_grid(F2, l2, "odd-pose");
    {
        ...
        check("lidar: beam (12,3) closed-form range",
              c.hit && c.box == IDX_WALL && std::fabs(got - closed_form) < TOL_RANGE,
        ...
        check("lidar: intensity == |N.V| on a known face", std::fabs(gi - std::fabs(ndv)) < 1e-4,
              fmt("%.6f, expected %.6f", gi, std::fabs(ndv)));
    }
    {
        long nonzero = 0;
        for (unsigned i = 0; i < 8; ++i)
            if (lshort->Buffer[i].range != 0.f)
                nonzero++;
        check("lidar: max_distance suppresses far returns", nonzero == 0,
              fmt("max_distance 10 m, wall at %.1f m, %ld/8 beams returned", WALL_FACE, nonzero));
    }
    ...
    printf("\n-- radar --\n");
    {
        ...
        for (unsigned j = 0; j < RH; ++j)
            for (unsigned i = 0; i < RW; ++i) {
                const double az = beam_az(i, RW, RHFOV), el = beam_el(j, RH, -0.5 * RVFOV, 0.5 * RVFOV);
                double dir[3];
                beam_ray(F, az, el, dir);
                HitRec c = analytic_trace(F.o, dir, BOXES, 1e-3, 100.0);
                const RadarReturn& g = rb->Buffer[(size_t)j * RW + i];
                if (std::fabs(g.azimuth - az) > 1e-5 || std::fabs(g.elevation - el) > 1e-5)
                    bad_ang++;
                if (!c.hit) {
                    sky++;
        ...
        check("radar: range matches analytic beam", bad == 0 && compared > 0,
              fmt("%ld beams, max |err| %.3e m, %ld over %.0e", compared, maxerr, bad, TOL_RANGE));
        check("radar: reported azimuth/elevation match the model", bad_ang == 0,
              fmt("%ld/%u beams off", bad_ang, RW * RH));
        if (sky > 0)
    ...
    printf("TIER 1 SUMMARY: %d passed, %d failed, %d known gap(s)%s\n", g_pass, g_fail, g_gap,
           g_strict ? " (strict: gaps are fatal)" : "");
    printf("RESULT: %s\n", g_fail == 0 ? "ANALYTIC RENDER MATH VERIFIED" : "*** FAILURES ***");
    printf("----------------------------------------------------------------\n");
    return g_fail == 0 ? 0 : 1;
}
