...
`ChSensor`, the filter graph, the buffer types, `ChScene` — runs on whichever ray-tracing backend
the build selected.
...
| `verify_render_math` | analytic ground truth for the ray tracer (see below) |
| `verify_dynamic_sensors` | accelerometer / gyroscope / magnetometer / GPS / tachometer on a driving vehicle |
...
`verify_render_math` and `verify_dynamic_sensors` print measurements and return non-zero on
failure, so they have their own `main()` and are registered with CTest directly rather than
through `build_utests`, which links `gtest_main`.
...
So `verify_render_math` derives every expected value from the scene geometry and the pinhole
...
Stochastic features — global illumination, area-light soft shadows, depth of field, sensor noise
— cannot be asserted this way. `utest_SEN_metal_stochastic` follows the pattern set by
