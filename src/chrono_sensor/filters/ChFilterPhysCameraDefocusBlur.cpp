...
CH_SENSOR_API void ChFilterPhysCameraDefocusBlur::Apply() {
    ...
    if (metal_phys_cam::DefocusBlur(m_buffer_in->Buffer.get(), m_buffer_out->Buffer.get(), img_w, img_h,
                                    m_focal_length, m_focus_dist, m_aperture_num, m_pixel_size, m_defocus_gain,
                                    m_defocus_bias))
        return;
#endif
