...
#ifdef CHRONO_HAS_METAL_RT
    if (metal_phys_cam::ExpsrToDV(m_buffer_in->Buffer.get(), m_buffer_out->Buffer.get(), m_buffer_in->Width,
                                  m_buffer_in->Height, m_ISO, m_expsr2dv_gains, m_expsr2dv_biases, m_expsr2dv_gamma,
                                  m_crf_type))
        return;
#endif
