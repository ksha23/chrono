...
void ChFilterVisualize::CreateGlfwWindow(std::string window_name) {
    ...
    glLoadIdentity();
    {  // framebuffer size (pixels), not window size (points): on a HiDPI display the two differ by
       // backingScaleFactor, and the image would otherwise fill only the lower-left corner.
        int fb_w, fb_h;
