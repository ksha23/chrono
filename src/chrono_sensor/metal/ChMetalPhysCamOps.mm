...
class Ops {
  public:
    ...
    bool run2(id<MTLComputePipelineState> pso,
              const void* in,
              void* out,
              size_t count,
              const PhysCamParams& params) {
  ...
  private:
    Ops() {
        @autoreleasepool {
            ...
            m_valid = psoDefocus && psoVignette && psoAggregate && psoNoise && psoDvGamma && psoDvSigmoid &&
                      psoDvLinear;
        }
    }
...
CH_SENSOR_API bool DefocusBlur(const void* buf_in,
                               void* buf_out,
                               unsigned int img_w,
                               unsigned int img_h,
                               float f,
...
CH_SENSOR_API bool Vignetting(void* buf_in_out,
                              unsigned int img_w,
                              unsigned int img_h,
                              float f,
                              float L,
...
CH_SENSOR_API bool Aggregator(void* buf_in_out,
                              unsigned int img_w,
                              unsigned int img_h,
                              float N,
                              float t,
...
CH_SENSOR_API bool ExpsrToDV(const void* buf_in,
                             void* buf_out,
                             unsigned int img_w,
                             unsigned int img_h,
                             float ISO,
    ...
    switch (crf_type) {
        case 0: pso = o.psoDvGamma; break;
        case 1: pso = o.psoDvSigmoid; break;
        case 2: pso = o.psoDvLinear; break;
        default: return false;  // caller reports the invalid CRF type
    }
