...
struct ChFilterMetalVisualize::Impl {
    ...
    void ensureWindow(int w, int h, const std::string& title) {
        ...
        v.wantsLayer = YES;
        CGFloat s = [window backingScaleFactor]; if (s < 1) s = 1;
    ...
    void display(const uint8_t* rgba, int w, int h, double simTime) { @autoreleasepool {
        ...
        if (!tex || texW != w || texH != h) {
            ...
            tex = [dev newTextureWithDescriptor:td]; texW = w; texH = h;
        }
...
ChFilterMetalVisualize::ChFilterMetalVisualize(int w, int h, std::string name)
    : ChFilter(name), m_w(w), m_h(h), m_win_name(std::move(name)) {
    p = new Impl();
}
ChFilterMetalVisualize::~ChFilterMetalVisualize() { delete p; }
...
void ChFilterMetalVisualize::Apply() {
    ...
    if (m_rgba8 && m_rgba8->Buffer) {
        w = m_rgba8->Width; h = m_rgba8->Height; simTime = m_rgba8->TimeStamp;
        rgba = reinterpret_cast<const uint8_t*>(m_rgba8->Buffer.get());
    } else if (m_semantic && m_semantic->Buffer) {
        w = m_semantic->Width; h = m_semantic->Height; simTime = m_semantic->TimeStamp; m_tmp.assign((size_t)w * h * 4, 0);
        for (size_t i = 0; i < (size_t)w * h; ++i) {
            unsigned short id = m_semantic->Buffer[i].class_id; unsigned char r=0,g=0,b=0;
            if (id) { unsigned int q = id * 2654435761u; r = 64 + (q & 127); g = 64 + ((q >> 8) & 127); b = 64 + ((q >> 16) & 127); }
            m_tmp[i*4]=r; m_tmp[i*4+1]=g; m_tmp[i*4+2]=b; m_tmp[i*4+3]=255;
        }
        rgba = m_tmp.data();
    } else if (m_di && m_di->Buffer) {
        w = m_di->Width; h = m_di->Height; simTime = m_di->TimeStamp; m_tmp.assign((size_t)w * h * 4, 0);
        float mx = 0; for (size_t i=0;i<(size_t)w*h;++i){ float rg=m_di->Buffer[i].range; if(rg>0&&rg<1e6f) mx=std::max(mx,rg);} if(mx<=0) mx=1;
        for (size_t i=0;i<(size_t)w*h;++i){ float rg=m_di->Buffer[i].range;
            unsigned char v = (rg>0&&rg<1e6f) ? (unsigned char)(255.f*std::max(0.f,1.f-rg/mx)) : 0;
            m_tmp[i*4]=v; m_tmp[i*4+1]=v; m_tmp[i*4+2]=v; m_tmp[i*4+3]=255; }
        rgba = m_tmp.data();
    }
