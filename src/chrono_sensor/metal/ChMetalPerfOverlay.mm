...
struct MetalPerfOverlay::Impl {
    ...
    void bakeText(id<MTLDevice> dev) {
        ...
        int W = (int)(sz.width + 0.5) + 26; if (W < 120) W = 120; if (W > 1200) W = 1200;
        int H = (int)(sz.height + 0.5) + 14; if (H < 34) H = 34;
        texW = W; texH = H;
        ...
        NSBezierPath* bg = [NSBezierPath bezierPathWithRoundedRect:NSMakeRect(2, 2, W - 4, H - 4)
                                                           xRadius:8
                                                           yRadius:8];
...
MetalPerfOverlay::MetalPerfOverlay() { p = new Impl(); }
MetalPerfOverlay::~MetalPerfOverlay() { delete p; }
...
void MetalPerfOverlay::Draw(void* device, void* encoder, double drawableW, double drawableH) {
    ...
    float quad[16] = {
        (float)left,  (float)top, 0.f, 0.f,
        (float)left,  (float)bot, 0.f, 1.f,
        (float)right, (float)top, 1.f, 0.f,
        (float)right, (float)bot, 1.f, 1.f,
    };
...
double MetalPerfOverlay::FPS() const { return p->fps; }
double MetalPerfOverlay::RTF() const { return p->rtf; }
