...
// cr::RenderScene, builds one BLAS per geometry + a TLAS of instances, and runs
...
struct ChMetalRTRenderer::Impl {
    ...
    id<MTLBuffer> uni, instBuf, instRot, tintBuf, matBuf, nBaseBuf, gNormal, gAlbedo, gUVBuf, gTexId, gOpacity, gRoughness, gMetallicBuf, gRoughTexIdBuf, gMetalTexIdBuf, gOpacityTexIdBuf, gTangentBuf, gNormalTexIdBuf, gSpecularBuf, gEmissiveBuf, gTexScaleBuf, gKsTexIdBuf, gKeTexIdBuf, tlasScratch, lightsBuf, instIdsBuf;
    id<MTLTexture> offTex, offTex2, whiteTex, envTex;
    bool envValid=false;

    void setEnvMap(const std::string& path){
        envValid=false; envTex=nil;
        if(path.empty()) return;
        int w=0,h=0,n=0; float* data=stbi_loadf(path.c_str(),&w,&h,&n,4);
        if(!data){ return; }
        ...
        stbi_image_free(data); envValid=true;
    }
    ...
    id<MTLBuffer> mkbuf(std::vector<float>& v) {
        if(v.empty()) return [dev newBufferWithLength:16 options:MTLResourceStorageModeShared];   // v.data() may be null when empty
    ...
    id<MTLBuffer> mkbufI(std::vector<int>& v) {
        if(v.empty()) return [dev newBufferWithLength:16 options:MTLResourceStorageModeShared];
        return [dev newBufferWithBytes:v.data() length:v.size()*4 options:MTLResourceStorageModeShared];
    }
    id<MTLTexture> offscreen(int w, int h) {
        if (offTex && (int)offTex.width==w && (int)offTex.height==h) return offTex;
        MTLTextureDescriptor* td=[MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA32Float width:w height:h mipmapped:NO];
        td.usage=MTLTextureUsageShaderWrite|MTLTextureUsageShaderRead; td.storageMode=MTLStorageModeShared;
        offTex=[dev newTextureWithDescriptor:td]; return offTex;
    }
    id<MTLTexture> loadTex(const std::string& path) {
        ...
        if(!src) return whiteTex; CGImageRef img=CGImageSourceCreateImageAtIndex(src,0,NULL); CFRelease(src); if(!img) return whiteTex;
    ...
    id<MTLAccelerationStructure> buildBLAS(id<MTLBuffer> vb,int tc,bool refit,id<MTLBuffer>* scr) {
        ...
        if(refit&&scr)*scr=[dev newBufferWithLength:MAX(sz.refitScratchBufferSize,16) options:MTLResourceStorageModePrivate];
        id<MTLCommandBuffer> cb=[queue commandBuffer]; id<MTLAccelerationStructureCommandEncoder> e=[cb accelerationStructureCommandEncoder];
        [e buildAccelerationStructure:as descriptor:pd scratchBuffer:bs scratchBufferOffset:0]; [e endEncoding]; [cb commit]; [cb waitUntilCompleted]; return as;
    }

    void build(const RenderScene& scene) { @autoreleasepool {
        geoms.clear(); blas.clear(); retain.clear(); texList.clear();
        std::vector<float> gn,ga,guv,go,grh,gmet,gtan,gspec,gemis,gts; std::vector<int> gt,grtx,gmtx,gopx,gntx,gktx,getx;
        for(auto& g: scene.geometries){
            ...
            gts.insert(gts.end(),g.texScale.begin(),g.texScale.end()); gktx.insert(gktx.end(),g.ksTexId.begin(),g.ksTexId.end()); getx.insert(getx.end(),g.keTexId.begin(),g.keTexId.end());
            guv.insert(guv.end(),g.uv.begin(),g.uv.end()); gt.insert(gt.end(),g.texId.begin(),g.texId.end());
            grtx.insert(grtx.end(),g.roughTexId.begin(),g.roughTexId.end()); gmtx.insert(gmtx.end(),g.metalTexId.begin(),g.metalTexId.end());
            ...
            id<MTLBuffer> scr=nil; id<MTLAccelerationStructure> as=buildBLAS(vb,gg.triCount,g.dynamic,&scr);
            gg.vbuf=vb; gg.scratch=scr; blas.push_back(as); retain.push_back(vb); geoms.push_back(gg);
        }
        gNormal=mkbuf(gn); gAlbedo=mkbuf(ga); gUVBuf=mkbuf(guv); gTexId=mkbufI(gt); gOpacity=mkbuf(go); gRoughness=mkbuf(grh); gMetallicBuf=mkbuf(gmet);
        ...
        float* tb=(float*)tintBuf.contents; uint32_t* mb=(uint32_t*)matBuf.contents; uint32_t* nb=(uint32_t*)nBaseBuf.contents; uint32_t* ib=(uint32_t*)instIdsBuf.contents;
        for(size_t i=0;i<scene.instances.size();++i){ auto& in=scene.instances[i]; tb[i*3]=in.tint[0];tb[i*3+1]=in.tint[1];tb[i*3+2]=in.tint[2]; mb[i]=in.mat; nb[i]=geoms[in.geom].triBase; ib[i*2]=in.classId; ib[i*2+1]=in.instanceId; }
        NSMutableArray* arr=[NSMutableArray array]; for(auto a:blas)[arr addObject:a];
        idesc=[MTLInstanceAccelerationStructureDescriptor descriptor]; idesc.instancedAccelerationStructures=arr;
        idesc.instanceDescriptorBuffer=instBuf; idesc.instanceDescriptorStride=64; idesc.instanceCount=(int)scene.instances.size();
    ...
    void updateDynamic(const RenderScene& scene) { @autoreleasepool {
        ...
        id<MTLCommandBuffer> cb=[queue commandBuffer]; id<MTLAccelerationStructureCommandEncoder> ae=[cb accelerationStructureCommandEncoder];
        for(size_t gi=0;gi<scene.geometries.size();++gi){ auto& g=scene.geometries[gi]; if(!g.dynamic) continue; auto& gg=geoms[gi];
            ...
            gd.vertexBuffer=gg.vbuf; gd.vertexStride=12; gd.vertexFormat=MTLAttributeFormatFloat3; gd.triangleCount=gg.triCount;
            MTLPrimitiveAccelerationStructureDescriptor* pd=[MTLPrimitiveAccelerationStructureDescriptor descriptor]; pd.geometryDescriptors=@[gd]; pd.usage=MTLAccelerationStructureUsageRefit;
    ...
    void fillUniforms(const MetalCameraParams& cam,int tw,int th,int numLights) {
        ...
        ui[21]=tw; ui[22]=th; ui[23]=(uint32_t)(cam.aa<1?1:cam.aa); ui[24]=(uint32_t)(numLights<0?0:numLights); ui[25]=(uint32_t)(cam.mode<0?0:cam.mode);
        uf[26]=cam.lidarHFov; uf[27]=cam.lidarVMin; uf[28]=cam.lidarVMax; uf[29]=cam.maxDist;
        ui[30]=(uint32_t)(cam.lidarSampleRadius<1?1:cam.lidarSampleRadius); uf[31]=cam.lidarHDiv; uf[32]=cam.lidarVDiv; ui[33]=(uint32_t)(cam.lidarReturnMode<0?0:cam.lidarReturnMode);
        ui[34]=(uint32_t)(cam.lensModel<0?0:cam.lensModel); uf[35]=cam.dk1; uf[36]=cam.dk2; uf[37]=cam.dk3;
        ui[38]=envValid?1u:0u;
        ui[39]=(uint32_t)(cam.bgMode<0?0:cam.bgMode);
        uf[40]=cam.bgZenith[0]; uf[41]=cam.bgZenith[1]; uf[42]=cam.bgZenith[2];
        uf[43]=cam.bgHorizon[0]; uf[44]=cam.bgHorizon[1]; uf[45]=cam.bgHorizon[2];
    ...
    void encodeTrace(id<MTLCommandBuffer> cb,id<MTLTexture> tex,int tw,int th) {
        ...
        [e setBuffer:uni offset:0 atIndex:0]; [e setBuffer:gNormal offset:0 atIndex:1]; [e setBuffer:gAlbedo offset:0 atIndex:2];
        [e setBuffer:instRot offset:0 atIndex:3]; [e setAccelerationStructure:tlas atBufferIndex:4]; [e setBuffer:nBaseBuf offset:0 atIndex:5];
        [e setBuffer:matBuf offset:0 atIndex:6]; [e setBuffer:tintBuf offset:0 atIndex:7]; [e setBuffer:gUVBuf offset:0 atIndex:8]; [e setBuffer:gTexId offset:0 atIndex:9];
        [e setBuffer:lightsBuf offset:0 atIndex:10]; [e setBuffer:instIdsBuf offset:0 atIndex:11]; [e setBuffer:gOpacity offset:0 atIndex:12]; [e setBuffer:gRoughness offset:0 atIndex:13]; [e setBuffer:gMetallicBuf offset:0 atIndex:14];
        ...
        [e dispatchThreadgroups:MTLSizeMake((tw+7)/8,(th+7)/8,1) threadsPerThreadgroup:MTLSizeMake(8,8,1)]; [e endEncoding];
    }
    void render(const MetalCameraParams& cam,const MetalLightGPU* lights,int numLights,int w,int h,float* out) { @autoreleasepool {
        ...
        if (numLights > 0) memcpy(lightsBuf.contents, lights, numLights*sizeof(MetalLightGPU));
        id<MTLTexture> tex=offscreen(w,h); fillUniforms(cam,w,h,numLights);
        id<MTLCommandBuffer> cb=[queue commandBuffer]; encodeTrace(cb,tex,w,h);
        ...
        if(cam.useDenoiser && dpso && cam.mode==0){   // portable despeckle/denoise pass (color mode only)
            ...
            id<MTLComputeCommandEncoder> de=[cb computeCommandEncoder]; [de setComputePipelineState:dpso];
            [de setTexture:tex atIndex:0]; [de setTexture:offTex2 atIndex:1]; [de setBytes:dims length:8 atIndex:0];
            [de dispatchThreadgroups:MTLSizeMake((w+7)/8,(h+7)/8,1) threadsPerThreadgroup:MTLSizeMake(8,8,1)]; [de endEncoding];
            outTex=offTex2;
        }
        [cb commit]; [cb waitUntilCompleted];
...
ChMetalRTRenderer::ChMetalRTRenderer(void* mtlDevice, void* mtlQueue) {
    ...
    if (!p->dev || !p->queue) return;
    NSError* e=nil; MTLCompileOptions* co=[MTLCompileOptions new];
    if(@available(macOS 14.0,*)) co.languageVersion=MTLLanguageVersion3_1; else co.languageVersion=MTLLanguageVersion2_4;
    id<MTLLibrary> lib=[p->dev newLibraryWithSource:@(kRaytraceMSL) options:co error:&e];
    if(!lib){ NSLog(@"Chrono::Sensor Metal RT shader error: %@",e); return; }
    ...
    MTLSamplerDescriptor* sd=[MTLSamplerDescriptor new]; sd.minFilter=MTLSamplerMinMagFilterLinear; sd.magFilter=MTLSamplerMinMagFilterLinear;
    sd.mipFilter=MTLSamplerMipFilterLinear; sd.sAddressMode=MTLSamplerAddressModeRepeat; sd.tAddressMode=MTLSamplerAddressModeRepeat;
    ...
    wd.usage=MTLTextureUsageShaderRead; p->whiteTex=[p->dev newTextureWithDescriptor:wd];
    uint8_t wp[4]={255,255,255,255}; [p->whiteTex replaceRegion:MTLRegionMake2D(0,0,1,1) mipmapLevel:0 withBytes:wp bytesPerRow:4];
}
