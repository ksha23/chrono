...
class CH_SENSOR_API ChScene {
  public:
    ...
    /// @param angle_range [rad], angle range of the spotlight falling off to zero.
    /// @param const_color whether to use constant color (no attenuation with distance)
    void ModifySpotLight(unsigned int light_ID, ChVector3f pos, ChColor color, float max_range, ChVector3f light_dir, float angle_falloff_start, float angle_range, bool const_color = true);
