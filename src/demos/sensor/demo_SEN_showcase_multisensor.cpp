...
static void SetSemantic(std::shared_ptr<ChBody> body, unsigned short cls, unsigned short inst) {
    ...
    for (auto& si : vm->GetShapeInstances()) {
        ...
        if (mats.empty()) {
            ...
            m->SetClassID(cls); m->SetInstanceID(inst);
            si.shape->AddMaterial(m);
        } else {
            for (auto& m : mats) { m->SetClassID(cls); m->SetInstanceID(inst); }
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    auto addObstacle = [&](std::shared_ptr<ChBody> body, ChVector3d pos, ChColor c) {
        ...
        auto m = chrono_types::make_shared<ChVisualMaterial>();
        m->SetDiffuseColor(c);
        m->SetClassID(next_class); m->SetInstanceID(next_class);  // one class per obstacle
        body->GetVisualModel()->GetShapeInstances()[0].shape->AddMaterial(m);
        sys.Add(body);
        ++next_class;
    };
    addObstacle(chrono_types::make_shared<ChBodyEasyBox>(1.2, 1.2, 1.5, 1000, true, false),
                ChVector3d(12.0, 3.0, 0.8), ChColor(0.85f, 0.18f, 0.18f));
    addObstacle(chrono_types::make_shared<ChBodyEasySphere>(0.9, 1000, true, false),
                ChVector3d(20.0, -3.0, 0.9), ChColor(0.18f, 0.75f, 0.28f));
    addObstacle(chrono_types::make_shared<ChBodyEasyCylinder>(ChAxis::Z, 0.6, 2.0, 1000, true, false),
                ChVector3d(28.0, 3.5, 1.0), ChColor(0.2f, 0.45f, 0.95f));
    addObstacle(chrono_types::make_shared<ChBodyEasyBox>(1.0, 2.2, 1.2, 1000, true, false),
                ChVector3d(36.0, -3.5, 0.6), ChColor(0.9f, 0.8f, 0.2f));
    addObstacle(chrono_types::make_shared<ChBodyEasySphere>(0.7, 1000, true, false),
