...
/// Whether an asset name contains any of the given fragments.
///
/// Substring rather than prefix: asset names carry an inconsistent vendor infix and pluralisation,
/// so "TrafficIsland" has to match "SM_McityTrafficIslands_s001". Matching on the prefix silently
/// left the traffic islands and the roundabout apron out of the ground surface, and a vehicle
/// driving onto either sank through geometry it could plainly see.
bool NameContains(const std::vector<std::string>& fragments, const std::string& name) {
...
bool ChSceneryModel::Load(ChSystem& sys, const std::string& manifest_file, const Options& options) {
    ...
    double t_assets = std::chrono::duration<double, std::milli>(
    ...
    unsigned int mesh_loads = 0, shape_count = 0;
    double load_ms = 0;
    ...
    for (const auto& inst : doc["instances"].GetArray()) {
        ...
        if (options.body_per_group) {
            ...
            if (options.tile_size > 0) {
                long tx = (long)std::floor(pos.x() / options.tile_size);
                long ty = (long)std::floor(pos.y() / options.tile_size);
                bkey += "_" + std::to_string(tx) + "_" + std::to_string(ty);
            }
        ...
        // Ground height comes from the same triangles that get drawn, transformed the same way,
        // so the surface a tire rests on and the surface on screen cannot drift apart.
        bool as_ground = options.ground_all
                             ? (!Listed(options.ground_exclude_groups, group) &&
                                !NameContains(options.ground_exclude, assets[ai].name))
                             : (!options.ground_assets.empty() &&
                                NameContains(options.ground_assets, assets[ai].name));
        if (as_ground) {
            if (!m_ground)
                m_ground = chrono_types::make_shared<ChMeshHeightField>();
            for (const auto& part : it->second) {
                const auto& mesh = part->GetMesh();
                const auto& verts = mesh->GetCoordsVertices();
                const auto& faces = mesh->GetIndicesVertices();
                const ChVector3d& s = part->GetScale();
                for (const auto& f : faces) {
                    m_ground->AddTriangle(
                        frame.TransformPointLocalToParent(verts[f.x()] * s),
                        frame.TransformPointLocalToParent(verts[f.y()] * s),
                        frame.TransformPointLocalToParent(verts[f.z()] * s));
                }
            }
        }
        m_num_instances++;
    ...
    double t_inst = std::chrono::duration<double, std::milli>(
                        std::chrono::steady_clock::now() - T0).count();
    T0 = std::chrono::steady_clock::now();
    std::cout << "    [assets " << (long)t_assets << " ms | instances " << (long)t_inst
              << " ms, of which mesh parse " << (long)load_ms << " ms | " << mesh_loads
              << " files, " << shape_count << " shapes]" << std::endl;

    if (m_ground)
        m_ground->Build(options.ground_cell_size);
    std::cout << "    [ground field " << (long)std::chrono::duration<double, std::milli>(
...
void ChSceneryModel::ReportTo(std::ostream& out) const {
    ...
    if (m_ground && m_ground->IsReady()) {
        ChVector3d lo, hi;
        m_ground->GetExtent(lo, hi);
        out << "    ground height field: " << m_ground->GetNumTriangles() << " triangles, "
            << (hi.x() - lo.x()) << " x " << (hi.y() - lo.y()) << " m, "
            << m_ground->GetMeanBucketOccupancy() << " tris/bucket\n";
    }
}
